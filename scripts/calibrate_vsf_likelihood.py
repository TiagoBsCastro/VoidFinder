#!/usr/bin/env python
"""Calibrate paired finder parameters by maximizing a binned VSF likelihood."""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Rank paired-halo void-finder parameters by Poisson likelihood of "
            "fixed-bin finder counts against VIDE VSF counts."
        )
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Minimum model count per bin used in the Poisson likelihood.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=Path("runs/void-statistics/vsf_likelihood_calibration.csv"),
...
def _row(result: PairedVsfLikelihoodResult, rank: int) -> dict[str, object]:
    radius_result = result.radius_result
    config = radius_result.config
    score_a = result.score_a
    score_b = result.score_b
    radius_a = score_a.radius_score
    radius_b = score_b.radius_score
    return {
        "rank": rank,
        "log_likelihood": result.total_log_likelihood,
        "negative_log_likelihood": result.total_negative_log_likelihood,
        "count_floor": result.count_floor,
        "is_degenerate": radius_result.is_degenerate,
        "linking_mode": radius_result.linking_mode,
        "linking_value": radius_result.linking_value,
        "source_a_linking_length_mpc_h": radius_result.source_a_linking_length_mpc_h,
        "source_b_linking_length_mpc_h": radius_result.source_b_linking_length_mpc_h,
        ...
        "source_a_clusters": radius_result.source_a_cluster_count,
        "source_b_clusters": radius_result.source_b_cluster_count,
        "target_a_protovoids": radius_result.protovoid_a_count,
        "target_b_protovoids": radius_result.protovoid_b_count,
        "target_a_edges": radius_result.edge_a_count,
        "target_b_edges": radius_result.edge_b_count,
        "total_density_l1": radius_result.total_density_l1_difference,
        "total_count_l1": radius_result.total_count_l1_difference,
        "total_median_radius_abs_error_mpc_h": _float(
            radius_result.total_median_radius_abs_error_mpc_h
        ),
        "total_in_bin_count_abs_error": radius_result.total_in_bin_count_abs_error,
        "target_a_log_likelihood": score_a.log_likelihood,
        "target_a_negative_log_likelihood": score_a.negative_log_likelihood,
        "target_a_finder_total": radius_a.finder_summary.count,
        "target_a_finder_in_bins": radius_a.size_score.predicted_void_count,
        "target_a_vide_total": radius_a.reference_summary.count,
        "target_a_vide_in_bins": radius_a.size_score.reference_void_count,
        "target_a_finder_median_mpc_h": _float(radius_a.finder_summary.median_mpc_h),
        "target_a_vide_median_mpc_h": _float(radius_a.reference_summary.median_mpc_h),
        "target_a_median_abs_error_mpc_h": _float(radius_a.median_radius_abs_error_mpc_h),
        "target_a_density_l1": radius_a.size_score.density_l1_difference,
        "target_a_count_l1": radius_a.size_score.count_l1_difference,
        "target_a_zero_in_bin": radius_a.is_zero_in_bin,
        "target_b_log_likelihood": score_b.log_likelihood,
        "target_b_negative_log_likelihood": score_b.negative_log_likelihood,
        "target_b_finder_total": radius_b.finder_summary.count,
        "target_b_finder_in_bins": radius_b.size_score.predicted_void_count,
        "target_b_vide_total": radius_b.reference_summary.count,
        "target_b_vide_in_bins": radius_b.size_score.reference_void_count,
        "target_b_finder_median_mpc_h": _float(radius_b.finder_summary.median_mpc_h),
        "target_b_vide_median_mpc_h": _float(radius_b.reference_summary.median_mpc_h),
        "target_b_median_abs_error_mpc_h": _float(radius_b.median_radius_abs_error_mpc_h),
        "target_b_density_l1": radius_b.size_score.density_l1_difference,
        "target_b_count_l1": radius_b.size_score.count_l1_difference,
        "target_b_zero_in_bin": radius_b.is_zero_in_bin,
    }


def write_csv(path: Path, results: Sequence[PairedVsfLikelihoodResult]) -> None:
    rows = [_row(result, rank) for rank, result in enumerate(results, start=1)]
    if not rows:
        raise SystemExit("No likelihood calibration rows were produced")
...
def _print_rows(results: Sequence[PairedVsfLikelihoodResult], *, top: int) -> None:
    print(
        "rank,degenerate,log_likelihood,linking,min_members,min_mass,a0,alpha,adj,"
        "A_in/VIDE,A_ll,B_in/VIDE,B_ll,density_l1"
    )
    for rank, result in enumerate(results[:top], start=1):
        radius_result = result.radius_result
        config = radius_result.config
        print(
            f"{rank},{radius_result.is_degenerate},"
            f"{result.total_log_likelihood:.8g},"
            f"{radius_result.linking_mode}:{radius_result.linking_value:g},"
            ...
            f"{result.score_a.radius_score.size_score.predicted_void_count}/"
            f"{result.score_a.radius_score.size_score.reference_void_count},"
            f"{result.score_a.log_likelihood:.8g},"
            f"{result.score_b.radius_score.size_score.predicted_void_count}/"
            f"{result.score_b.radius_score.size_score.reference_void_count},"
            f"{result.score_b.log_likelihood:.8g},"
            f"{radius_result.total_density_l1_difference:.8e}"
        )
