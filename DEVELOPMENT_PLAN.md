...
- Added full-MCMC preflight diagnostics, with an explicit
  `--fail-on-rejected-preflight` switch for aborting all-rejected starts before
  a long `emcee` run.
...
/home/tcastro/miniforge3/envs/voidfinder/bin/python scripts/optimize_n256_full_algorithm_mcmc.py --vide-variant untrimmed --position-mode initial --walkers 16 --center-weight 0 --allow-degenerate --fail-on-rejected-preflight --steps 8 --burn-in 2 --thin 1 --output-prefix /tmp/n256_full_mcmc_initial_failfast_smoke
